...
from my_robot_interfaces.msg import turtle_array
from my_robot_interfaces.srv import catch_turtle
...
class TurtleControllerNode(Node):
    def __init__(self):
        ...
        self.alive_turtles_subscriber_ = self.create_subscription(
            turtle_array, "alive_turtles", self.callback_alive_turtles, 10)
    ...
    def call_catch_turtle_server(self, turtle_name):
        client = self.create_client(catch_turtle, "catch_turtle")
        while not client.wait_for_service(1.0):
            self.get_logger().warn("Waiting for Server...")

        request = catch_turtle.Request()
