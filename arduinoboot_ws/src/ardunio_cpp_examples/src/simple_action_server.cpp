...
public:
    ...
    : Node("simple_action_server", options)
  {
    ...
    action_server_ = rclcpp_action::create_server<arduniobot_msgs::action::Fibonacci>(
        this, "fibonacci",
...
private:
  rclcpp_action::Server<arduniobot_msgs::action::Fibonacci>::SharedPtr action_server_;

  rclcpp_action::GoalResponse goalCallback(
      const rclcpp_action::GoalUUID& uuid,
      std::shared_ptr<const arduniobot_msgs::action::Fibonacci::Goal> goal)
  {
  ...
  rclcpp_action::CancelResponse cancelCallback(
      const std::shared_ptr<rclcpp_action::ServerGoalHandle<arduniobot_msgs::action::Fibonacci>> goal_handle)
  {
  ...
  void acceptedCallback(
      const std::shared_ptr<rclcpp_action::ServerGoalHandle<arduniobot_msgs::action::Fibonacci>> goal_handle)
  {
    // Spin up a new thread to handle goal execution
    std::thread{ std::bind(&SimpleActionServer::execute, this, _1), goal_handle }.detach();
  }

  void execute(const std::shared_ptr<rclcpp_action::ServerGoalHandle<arduniobot_msgs::action::Fibonacci>> goal_handle)
  {
    RCLCPP_INFO(get_logger(), "Executing goal");
    rclcpp::Rate loop_rate(1);
    const auto goal = goal_handle->get_goal();
    auto feedback = std::make_shared<arduniobot_msgs::action::Fibonacci::Feedback>();
    auto& sequence = feedback->partial_sequence;
    sequence.push_back(0);
    sequence.push_back(1);
    auto result = std::make_shared<arduniobot_msgs::action::Fibonacci::Result>();
